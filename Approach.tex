...
Based on our aim, we can read this matrix as follows:
 \begin{itemize}
     ...
     Moreover, \autoref{fig:evolutionMatrixApproach} is an excellent example to understand why we cannot only rely on the name of the file to identify a system's entity. 
...
In addition, \autoref{fig:evolutionMatrixApproach} also provides an example of how we reconstruct the state of the system after a commit. 
As we have seen, each ProjectVersion does not represent a snapshot of the system.
Instead, it represents only the difference in changes made to the previous version. 
To reconstruct the state of the system at a specific version, we need to consider, for each FileHistory, the last change made before that version. 
Under those circumstances, for each FileHistory, we need to go back in time until we find the rightmost change. Of course, if the rightmost difference was a DELETE, we have to ignore the related FileHistory.
In \autoref{fig:evolutionMatrixApproach}, the state of the system after COMMIT 10 is composed by the FileHistories that holds a row with a blue square. 

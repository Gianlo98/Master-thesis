...
Displaying lots of information on the screen might not be an efficient way to gather crucial information about the evolution of a software system. We provide an auditorial representation of an AnimationFrame to support its understanding. The goal is to transmit hidden information such as the number of files added or removed through audio notes. 

Music theory defines many concepts to study which aural phenomenons compose music, for example, beats per minute (BPM), pitch, or note.
...
\begin{itemize}
	...
	\item Note's pitch: the quality that makes it possible to judge sounds as \quotes{higher} and \quotes{lower} in a sense associated with musical melodies.
...
\end{itemize}
The metrics can be easily extended depending on the goal of the melody. For example, we can create a new metric to play how many Java files were added. The implementation must be done with a tool that provides different sounds for each metric. A synthesizer is a component that generates audio signals. For example, a synthesizer can play the sound of a guitar, whereas another synthesizer can play the sound of a piano. The idea is to allow the user to help understand what is happening on the displayed AnimationFrame by recognizing the instrument through the sound. 

The output of this approach is a piece of music representing a system's evolution. The evolution of a system was depicted as a sequence of AnimationFrames. Therefore, we map a measure to each AnimationFrame. The total length of the evolution visualization depends on the tempo and the size of a measure. 
For example, if we display a view with 60 AnimationFrames, each one mapped to a measure long of four beats playing at 60 BPM, to represent the view, we need four minutes ($240$ beats played at $60$ BPM). The software system's auralization might suffer from this approach because, with a high BPM, the time between two AnimationFrames might not be sufficient. To overcome this issue, a fixed interval such as one second or two AnimationFrames can be set. Consequently, the same view of 60 AnimationFrames now needs one minute to be played. Within each second, for each chosen metric, we play sounds with an amplitude and a speed that depends upon the metric value. 

To summarize, in our auditorial approach, we support the visualization of a project's view with a musical representation of each AnimationFrames. Metrics collected during the analysis are used in an orchestral composition to control the tempo, the note, and the amplitude. An exemplary implementation should provide a wide range of sounds to differentiate the adopted metric values. We can choose to play one AnimationFrame every second or map it to a measure. 

...
\subsection{Analysis}
The Core provides abstract classes to standardize the exchange of analysis results. The first class that the model defines is \textbf{AnalysisWorkDescriptor}.
...
The reason for this implementation choice is to allow multiple threads to run in parallel analyses without analyzing the same commit numerous times. We described this approach in \autoref{s:EvolutionModel}
...
If a file changes its path in the middle of two analysis results, we have an entity with the old path in the first analysis and an entity with a new path in the second analysis. 
...
Algorithm \autoref{algo:mapping} was developed to create a mapping between FileHistories when we need to join them. It takes as input a list of already discovered FileHistories and a \\ \texttt{ProjectAnalysisResult}, and it returns a dictionary.
...
\autoref{fig:ServertClientFlow} explains how a View is retrieved through a GraphQL endpoint with a sequence diagram. Once the client, in this case, Visual Inspector, specifies a \texttt{viewSpecification}, it makes a POST request to the 
...
\begin{figure}[h]
    \center
    \fbox{\includegraphics[width=\textwidth]{SYNUI-List.png}}

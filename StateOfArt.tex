...
Around the end of the 80s, Müller et al. \cite{Mueller1988} released Rigi (\autoref{fig:Rigi}), a tool used to visualize large programs.
...
One year later, De Pauw et al. \cite{DePauw1993} introduced Jinsight (\autoref{fig:Jinsight}), a tool able to provide animated views of object-oriented systems' behavior. 
...
The second technique was the 3D wheel glyph (\autoref{fig:Chuan2}); it encoded the same attributes of the time wheel, and additionally, it used the height to encode time. 
...
The shape of the matrix could also be used to infer various evolutionary patterns; An example of them can be seen in \autoref{fig:EvolutionMatrix2}.
...
RelVis, the tool they developed (\autoref{fig:RelVis}), was able to depict a multivariate visualization of the evolution of a system.
...
They also presented the Evolution Radar \cite{DAmbros2006a} (\autoref{fig:Evoradar}), a novel approach to visualize module-level and file-level logical coupling information.
